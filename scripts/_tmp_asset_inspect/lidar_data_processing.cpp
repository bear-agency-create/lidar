...
Lidar_Data_Processing::Lidar_Data_Processing()
{
  ...
  FirstSampleAngle = 0; //< Start sampling angle
  LastSampleAngle = 0;  //< End sampling angle
  ...
  PackageSampleBytes = 2; //< Number of laser points in a package
  package_index = 0;
  recvNodeCount = 0;
  start_t = 0;
...
/************************************************************************/
/*  Issue control command to LiDAR                                     */
/************************************************************************/
...
result_t Lidar_Data_Processing::waitPackage(node_info *node, uint32_t timeout)
{
  ...
  uint32_t startTs = getms();
  uint32_t waitTime = 0;
  uint8_t *packageBuffer = (node_lidar.lidar_general_info.m_intensities) ? (uint8_t *)&node_lidar.scan_packages.package.package_Head : (uint8_t *)&node_lidar.scan_packages.packages.package_Head;
...
result_t Lidar_Data_Processing::waitPackage_coin(node_info *node, uint32_t timeout)
{
  ...
  if (CheckSumResult)
  {
    (*node).distance_q2 = (node_lidar.scan_packages.package_coin.data[package_Sample_Index].d0 +
                           (node_lidar.scan_packages.package_coin.data[package_Sample_Index].d1 * 256)) &
                          0x3FFF;
...
result_t Lidar_Data_Processing::waitScanData(node_info *nodebuffer, size_t &count, uint32_t timeout)
{
  ...
  /*Timeout handling and point count check*/
  while ((waitTime = getms() - startTs) <= timeout && recvNodeCount < count)
  {
    ...
    if (node.sync_flag & LIDAR_RESP_MEASUREMENT_SYNCBIT)
    {
      ...
      /*Data amount of the packet*/
      size_t PackageSize = (node_lidar.lidar_general_info.m_intensities ? INTENSITY_NORMAL_PACKAGE_SIZE : NORMAL_PACKAGE_SIZE);
