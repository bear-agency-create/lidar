...
/* LiDAR startup status judgment function */
bool lidar_state_judgment()
{
	static bool status_judge = false;	  // Overall status judgment
	static bool lidar_flush = false;	  // Whether LiDAR start command has been sent
	static bool wait_speed_right = false; // Whether speed adjustment info received
	static bool lidar_start_flag = false; // LiDAR feedback flag after start command

	static uint64_t lidar_status_time = 0; // Time when start or restart command received
	...
	{
		printf("Abnormal status, restarting %lld\n", lidar_status_time);
	...
	if (node_lidar.lidar_status.lidar_ready && !wait_speed_right)
	{
		...
		if (current_times() - lidar_status_time > 1000 && !lidar_flush)
		{
			...
			{
			case M1C1_Mini_v1:
				printf("Starting LiDAR V1\n");
				node_lidar.serial_port->write_data(start_lidar, 4);
				wait_speed_right = true;
				break;

			case M1C1_Mini_v2:
				printf("Starting LiDAR V2/X2\n");
				node_lidar.serial_port->write_data(start_lidar, 4);
				wait_speed_right = true;
				break;

			case M1CT_TOF:
				printf("Starting TOF LiDAR\n");
				node_lidar.serial_port->write_data(start_lidar, 4);
				if (!node_lidar.lidar_data_processing.wait_start_reply())
				{
					printf("No start feedback received, sending again\n");
					node_lidar.serial_port->write_data(start_lidar, 4);
				}
				wait_speed_right = true;
				break;
			...
			for (size_t pos = 0; pos < count; ++pos)
			{
				if (local_buf[pos].sync_flag & LIDAR_RESP_MEASUREMENT_SYNCBIT)
				{
					if ((local_scan[0].sync_flag & LIDAR_RESP_MEASUREMENT_SYNCBIT))
					{
						...
						/*Frequency abnormal for more than 30 seconds, trigger abnormal status*/
...
bool initialize()
{
	...
	{
		// Calculate distance from LiDAR installation position to robot edge
	...
	// Get time for serial port to receive each byte
...
int node_start()
{
	...
	/*Thread for reading LiDAR data*/
	thread t1(read_forever);
	t1.detach();

	/*Thread for processing LiDAR data*/
	/*
