...
/* Process the latest scan data from LiDAR */
...
int main(int argc, char **argv)
{
	...
	node->declare_parameter("port", "/dev/ttyLIDAR");
	...
	while (rclcpp::ok())
	{
		...
		{
			auto scan_msg = std::make_shared<sensor_msgs::msg::LaserScan>(); // LiDAR scan data
			sensor_msgs::msg::PointCloud2 pclMsg;							 // LiDAR point cloud data
